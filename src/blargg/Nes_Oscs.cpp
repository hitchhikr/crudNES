...
static const char duty_table [4] =
{
    1, 2, 4, 6
};
...
{
    const int sweep_overflows = period + (sweep_negate ^ (period >> sweep_shift)) >= 0x800;
...
void Nes_Dmc::recalc_irq()
{
    ...
    {
        irq = apu->last_time + delay + ((duration - 1) * 8 + bits_remain - 1) * nes_time_t (period) + 1;
    }
    if ( irq != next_irq )
    {
        next_irq = irq;
...
void Nes_Noise::run( nes_time_t time, nes_time_t end_time )
{
    ...
    if ( time < end_time )
    {
        ...
        else
        {
            ...
            const Blip_Buffer::resampled_time_t resampled_period = output->resampled_duration( period );
