...
//	BOOST_STATIC_ASSERT( -32768 < range && range < 32767 );
    ...
    public:
        ...
        {
            impulse.volume_unit( v * (1.0 / abs_range) );
        }
        ...
        // not documented
        void offset_resampled( Blip_Buffer::resampled_time_t time, int delta, Blip_Buffer* blip_buf ) const;
...
class Blip_Wave
{
    ...
    public:
        ...
        blip_time_t time() const
        {
            return time_;
        }
        void time( blip_time_t t )
        {
            time_ = t;
        }
        ...
        Blip_Buffer::resampled_time_t time, int delta, Blip_Buffer* blip_buf ) const
{
    ...
    enum
    {
        const_offset = Blip_Buffer::widest_impulse_ / 2 - width / 2
    };
    pair_t* buf = (pair_t*) &blip_buf->buffer_ [const_offset + sample_index];
    
    enum
    {
        shift = BLIP_BUFFER_ACCURACY - blip_res_bits_
    };
    enum
    {
        mask = res * 2 - 1
