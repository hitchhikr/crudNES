...
int Blip_Buffer::sample_rate( long sps, int msec )
{
	samples_per_sec = sps;
	if ( clocks_per_sec )
    {
	...
	const size_t max_size = (0xFFFFFFFF >> BLIP_BUFFER_ACCURACY) + 1 - max_size_adj;
...
{
	clocks_per_sec = cps;
	factor_ = (unsigned long) floor( (double) samples_per_sec / cps * (1L << BLIP_BUFFER_ACCURACY) + 0.5 );
...
size_t Blip_Buffer::read_samples( blip_sample_t* out, size_t max_samples, int stereo )
{
	...
	if ( !stereo )
    {
        ...
        {
			*out++ = (blip_sample_t) ((accum >> accum_fract) ^ 0x8000);
...
{
	long offset = ((long) unit << impulse_bits) - impulse_offset * unit + (1 << (impulse_bits - 1));
	imp_t* imp = imp_in;
	imp_t* fimp = impulse;
	for ( int n = res / 2 + 1; n--; )
	{
...
void Blip_Impulse_::volume_unit( double new_unit )
{
	...
	offset = (uint32_t) (0x10001 * floor( volume_unit_ * 0x10000 + 0.5 ));
