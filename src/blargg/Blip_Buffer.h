...
class Blip_Buffer
{
    public:
        ...
        int sample_rate( long samples_per_sec );
        int sample_rate( long samples_per_sec, int msec_length );
        ...
        size_t read_samples( blip_sample_t* dest, size_t max_samples, int stereo = FALSE );
...
{
    offset_ += t * factor_;
    assert(( "Blip_Buffer::end_frame(): Frame went past end of buffer", samples_avail() <= buffer_size_ ));
}

inline void Blip_Buffer::remove_silence( size_t count )
{
    assert(( "Blip_Buffer::remove_silence(): Not that many samples available to remove", count <= samples_avail() ));
    offset_ -= resampled_time_t (count) << BLIP_BUFFER_ACCURACY;
}

inline int Blip_Buffer::sample_rate( long samples_per_sec )
{
