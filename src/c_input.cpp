...
void c_input :: write_strobe (__UINT_8 value)
{
    if (value == BIT_0)
    {
        full_strobe_1 = FALSE; half_strobe_1 = TRUE; full_strobe_2 = FALSE; half_strobe_2 = TRUE;
    }
    else if (value == 0)
    {
...
__UINT_8 c_input :: handle_key (void)
{
    ...
    if (key [KEY_F7])
    {
        nes->o_state->DecrementSlot ();
    }
    if (key [KEY_F8])
    {
        nes->o_state->IncrementSlot ();
    }
    if (key [KEY_F10])
    {
        nes->o_cpu->reset ();
...
void c_input :: handle_joypad (__UINT_8 controller)
{
    ...
    else
    {
        ...
        if (!poll_joystick())
        {
            if (joy [o_machine->read_from_second_pad].button [0].b)
            {
                last_press_2 |= BIT_0;
            }
            if (joy [o_machine->read_from_second_pad].button [1].b)
            {
                last_press_2 |= BIT_1;
            }
            if (joy [o_machine->read_from_second_pad].button [2].b)
            {
                last_press_2 |= BIT_2;
            }
            if (joy [o_machine->read_from_second_pad].button [3].b)
            {
                last_press_2 |= BIT_3;
            }
            if (joy [o_machine->read_from_second_pad].button [7].b)
            {
                last_press_2 |= BIT_3;
            }
            if (joy [o_machine->read_from_second_pad].stick [0].axis [0].d1)
            {
                last_press_2 |= BIT_6;
            }
            if (joy [o_machine->read_from_second_pad].stick [0].axis [0].d2)
            {
                last_press_2 |= BIT_7;
            }
            if (joy [o_machine->read_from_second_pad].stick [0].axis [1].d1)
            {
                last_press_2 |= BIT_4;
            }
            if (joy [o_machine->read_from_second_pad].stick [0].axis [1].d2)
            {
                last_press_2 |= BIT_5;
            }
        }
