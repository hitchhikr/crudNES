...
extern "C"
{
    void RenderTileline (__UINT_8 *destination, __UINT_8 *Solid, __UINT_16 *Palette, __UINT_8 *VRam,
                         s_rendering_information *information, __UINT_16 Pattern, __UINT_16 Attribute);
}
...
c_nes_ppu :: c_nes_ppu (void)
{
    ...
    switch (nes->o_rom->information ().mirroring)
    {
        ...
        default:
            set_mirroring (_2C02_HORIZONTAL_MIRRORING);
    }
...
void c_nes_ppu :: run_accurate ()
{
    ...
    if (oam_inrange_sp_backup)
    {
        ...
        for (__UINT_16 spr_tmp_index = 0; spr_tmp_index < (oam_inrange_sp_backup << 3); spr_tmp_index += 8)
        {
            *((__UINT_32 *) (sprite_data)) = BUFOAM.read_dword (spr_tmp_index);
            ...
            {
                render_po_tileline_16 (sprite_data [SPR_X], info.scanline, tileline, sprite_data [SPR_ATTRIBUTE] & 3,
                                       sprite_data [SPR_ATTRIBUTE] & BIT_6, sprite_data [SPR_ATTRIBUTE] & BIT_5);
            }
            else
            {
                render_sp_tileline_16 (sprite_data [SPR_X], tileline, sprite_data [SPR_ATTRIBUTE] & 3,
                                       sprite_data [SPR_ATTRIBUTE] & BIT_6, sprite_data [SPR_ATTRIBUTE] & BIT_5);
            }
        }
    ...
    for (bTile = 1; bTile < 32; bTile ++, accurate_tile_index ++)
    {
        if (nes->is_mmc2_vrom)
        {
            int changeline = info.bg_pattern_base | (nt_set[bTile + 1] << 4);
            if((changeline & 0xf00) == 0xf00)
            {
                nes->o_mapper->update (&changeline);
            }
    ...
    //Dead cycle
    if (info.scanline || nes->o_cpu->is_pal ())
    {
        nes->o_cpu->run_cycles (1);
    }
    else 
    ...
    if (5 == nes->o_rom->information ().mapper)
    {
...
void c_nes_ppu :: render_bg_tileline16 (void)
{
    ...
    if (bTile) 
    {
        _2C02_UNPACK_TILELINE_SLOW (low_pattern_set [accurate_tile_index - 1], high_pattern_set [accurate_tile_index - 1],
                                    (unpacked_pixel_data + ((accurate_tile_index - 1) << 3)));
    }
    else
    {
    ...
    {
        nes->o_cpu->run_cycles (1);
        if (bTile == 32 && nes->o_rom->information ().mapper != 5)
        {
            nes->o_mapper->h_blank ();
        }
    }
    ...
    if (bTile != 31)
    {
        ...
        attribute = at_set [(unpacked_pixel_index >> 3)];
        _2C02_DRAW_ACCURATE_PIXEL (__UINT_16, unpacked_pixel_data, final_color, attribute, destination, solid, unpacked_pixel_index);
        is_mmc5_vrom = ((nes->o_rom->information ().mapper == 5) && 
                         nes->o_ppu->get_flag (CTL_1, BIT_5) && 
                         nes->o_ppu->get_flag (CTL_2, BIT_3 | BIT_4)) ? TRUE : FALSE;
        FetchLowPatternDataAddress ();
        nes->o_cpu->run_cycles (1);
        attribute = at_set [(unpacked_pixel_index >> 3)];
        _2C02_DRAW_ACCURATE_PIXEL (__UINT_16, unpacked_pixel_data, final_color, attribute, destination, solid, unpacked_pixel_index);
...
void c_nes_ppu :: render_po_tileline_16 (__UINT_16 x_offset, __UINT_16 y_offset, __UINT_16 line,
                                         __UINT_32 attribute, __BOOL SPXFlip, __BOOL priority)
{
    ...
    if (!(registers [CTL_2] & BIT_2) && (x_offset < 8))
    {
        tileline_offset = 8 - x_offset;
        x_offset = 8;
    }
    ...
    {
        if (!(pixel_data [tileline_offset ^ flip_mask]))
        {
            continue;
        }
        solid_pixel_lut [x_offset] |= (BIT_1 | BIT_2 | BIT_4);

        _2C02_DRAW_PIXEL (x_offset, y_offset, _2C02_NES_COLOR (__UINT_16, sprite_palette, attribute |
                                                               pixel_data [tileline_offset ^ flip_mask]));
        
        if (priority)
        {
            solid_pixel_lut [x_offset] |= BIT_3;
        }
    }
}
...
void c_nes_ppu :: render_sp_tileline_16 (__UINT_16 x_offset, __UINT_16 tileline, __UINT_32 attribute, __UINT_8 flip_mask, __BOOL priority)
{
    ...
    __UINT_8 tileline_offset = 0;
    if ((x_offset < 8) && !(registers [CTL_2] & BIT_2))
    {
        tileline_offset = 8 - x_offset; x_offset = 8;
    }
    ...
    for (; tileline_offset < 8; tileline_offset ++, x_offset++)
    {
        ...
        if (priority)
        {
            ...
            {
                _2C02_DRAW_PIXEL (x_offset, info.scanline, _2C02_NES_COLOR (__UINT_16, sprite_palette, (attribute << 2) |
                                                                            pixel_data [tileline_offset ^ flip_mask]));
...
{
    if (!get_flag (CTL_2, BIT_3 | BIT_4) || !oam_inrange_sp_backup)
    {
        return;
    }
    ...
    *((__UINT_32 *) (sprite_data)) = TMPOAM.read_dword (tmp_oam_index);
    ...
    if (sprite_data [SPR_ATTRIBUTE] & BIT_7)
    {
        sprite_data [SPR_TILELINE] ^= (0xf >> (get_flag (CTL_1, BIT_5) ? 0 : 1));
    }
    ...
    switch (address & 7)
    {
        ...
        case 0x07:
        {
                ...
                && get_flag (CTL_2, BIT_4 | BIT_3))
            {
                ...
                {
                    nes->general_log.f_write ("s", "PPU: WARNING: Invalid read at $2007.\r\n");
                }
                return 0x40;
            }
            ...
            if (address < 0x2000)
            {
                set_read_buffer (read_chr_ram (address));
            }
            else if (address < 0x3000)
            {
                set_read_buffer (read_nt_byte (address));
            }
            else if (address < 0x3f00)
            {
                set_read_buffer (read_nt_byte (address - 0x1000));
            }
            else if (address < 0x4000)
            {
                value = background_palette [address & 0x1f];
            }
    ...
    switch (address & 7)
    {
        case 0x07:
        {
                ...
                get_flag (CTL_2, BIT_4 | BIT_3))
            {
                if (nes->o_cpu->is_tracer_on ())
                {
                    nes->general_log.f_write ("sbs", "PPU: WARNING: Ignoring invalid write at $2007 Data: ", value, ".\r\n");
                }
                return;
            }
            ...
            if (address < 0x2000)
            {
                write_chr_ram (address, value);
            }
            else if (address < 0x3000)
            {
                write_nt_byte (address, value);
            }
            else if (address < 0x3f00)
            {
                write_nt_byte ((reg.contents - 0x1000) & 0x3fff, value);
            }
            else if (address < 0x4000)
            {
                value &= 0x7f;
                ...
                {
                    background_palette [address & 0x1f] = value;
                }
            }
        ...
        case 0x00:
            ...
            if (!get_flag (CTL_1, BIT_7) && (value & BIT_7) && get_flag (STAT, BIT_7))
            {
                if (nes->o_cpu->is_tracer_on ())
                {
                    nes->general_log.f_write ("sds", "PPU: WARNING: Extra NMI requested at scanline #", info.scanline, ".\r\n");
                }
...
void c_nes_ppu :: load_state (c_tracer &o_reader, e_save_state Type)
{
    switch (Type)
    {
        ...
        case OTHER:
            ...
            if (nes->o_rom->information ().mapper != 5)
            {
                set_mirroring (mirroring_mode);
            }
            else
            {
                ((c_mapper_005 *)(nes->o_mapper))->set_mirroring ();
            }
            break;
    }
}
