...
static inline int GetResult1(uint32 A, uint32 B, uint32 C, uint32 D, uint32 E)
{
	...
	int y = 0;
	int r = 0;
	if (A == C) x_offset+=1; else if (B == C) y+=1;
	if (A == D) x_offset+=1; else if (B == D) y+=1;
	if (x_offset <= 1) r+=1; 
	if (y <= 1) r-=1;
	return r;
}
...
{
    register uint32 x_offset = ((A & qcolorMask) >> 2) +
                        ((B & qcolorMask) >> 2) +
                        ((C & qcolorMask) >> 2) +
                        ((D & qcolorMask) >> 2);
...
//                                       M|N O|P
			...
			_farnspokel (dP + dstPitch, product1);
...
static uint32 Bilinear (uint32 A, uint32 B, fixed x_offset)
{
	...
	areaB = (x_offset >> 11) & 0x1f; //reduce 16 bit fraction to 5 bits
	areaA = 0x20 - areaB;
...
static uint32 Bilinear4 (uint32 A, uint32 B, uint32 C, uint32 D, fixed x_offset, fixed y)
{
	unsigned long areaA, areaB, areaC, areaD;
	unsigned long result, xy;

	x_offset = (x_offset >> 11) & 0x1f;
	y = (y >> 11) & 0x1f;
	xy = (x_offset*y) >> 5;
...
void Scale_2xSaI(uint8 *srcPtr, uint32 srcPitch,
	     uint8 *deltaPtr,
	     BITMAP *dstBitmap, int width, int height)
{
    uint32 dstPitch = dstBitmap->w * 2;
	...
	dw = wfinish/(dstBitmap->w-1);
	hfinish = (height-1) << 16; //convert to fixed point
	dh = hfinish/(dstBitmap->h-1);
    ...
    for (h = 0; h < hfinish; h += dh)
	{
	        ...
	        dP = bmp_write_line (dstBitmap, line);
	        y2 = 0x10000 - y1;
		...
		for (;w < wfinish;)
		{
			...
			uint32 position, product1;/*, product2;*/
...
/*1*/			if (A == D && B != C)
			{
			 	...
			 	if (y1 <= f1 && A == J && A != E) // close to B
				{
					...
					product1 = Bilinear (A, B, a1);
                            	...
                            	if (y1 >= f1 && A == G && A != L) // close to C
				{
					a1 = y1 - f1;
					product1 = Bilinear (A, C, a1);
				}
				else
			 	if (x1 >= f2 && A == E && A != J) // close to B
				{
					a1 = x1 - f2;
					product1 = Bilinear (A, B, a1);
				}
				else
                            	if (x1 <= f2 && A == L && A != G) // close to C
				{
					a1 = f2 - x1;
					product1 = Bilinear (A, C, a1);
				}
...
/*2*/			if (B == C && A != D)
			{
			 	...
			 	if (y2 >= f1 && B == H && B != F) // close to A
				{
					...
					product1 = Bilinear (B, A, a1);
                            	...
                            	if (y2 <= f1 && B == I && B != K) // close to D
				{
					a1 = f1 - y2;
					product1 = Bilinear (B, D, a1);
				}
				else
			 	if (x2 >= f2 && B == F && B != H) // close to A
				{
					a1 = x2 - f2;
					product1 = Bilinear (B, A, a1);
				}
				else
                            	if (x2 <= f2 && B == K && B != I) // close to D
				{
					a1 = f2 - x2;
					product1 = Bilinear (B, D, a1);
				}
